...
Return_t dmMotorSendMessage(DM_Motor_t* motor) {
    ...
    pos_tmp = float_to_uint(motor->command_interfaces.pos, -motor->p_max,
                            motor->p_max, 16);
    vel_tmp = float_to_uint(motor->command_interfaces.vel, -motor->v_max,
                            motor->v_max, 12);
    kp_tmp = float_to_uint(motor->command_interfaces.kp, motor->kp_min,
                           motor->kp_max, 12);
    kd_tmp = float_to_uint(motor->command_interfaces.kd, motor->kd_min,
                           motor->kd_max, 12);
    tor_tmp = float_to_uint(motor->command_interfaces.t, -motor->p_max,
                            motor->p_max, 12);
...
void dmMotorInfoUpdate(DM_Motor_t* motor, uint8_t* data) {
    ...
    motor->state_interfaces.position = uint_to_float(
        motor->state_interfaces.pos, -motor->p_max, motor->p_max, 16);

    motor->state_interfaces.vel = ((uint16_t)data[3] << 4) | (data[4] >> 4);
    motor->state_interfaces.velocity = uint_to_float(
        motor->state_interfaces.vel, -motor->v_max, motor->v_max, 12);

    motor->state_interfaces.t = ((uint16_t)(data[4] & 0xf) << 8) | (data[5]);
    motor->state_interfaces.torque = uint_to_float(
        motor->state_interfaces.t, -motor->t_max, motor->t_max, 12);
