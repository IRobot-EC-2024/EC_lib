...
static void usartDMARestart(Usart_Device_t *instance) {
  ...
  case 2:
    HAL_UARTEx_ReceiveToIdle_DMA_double(
        instance->usart_handle, instance->rx_buff2[0], instance->rx_buff2[1],
        instance->rx_len);
    __HAL_DMA_DISABLE_IT(instance->usart_handle->hdmarx, DMA_IT_HT);
    break;
  default:
...
Return_t usartSendMessage(Usart_Device_t *instance, uint8_t *message,
  ...
  case USART_TRANSFER_DMA:
    if (HAL_UART_Transmit_DMA(instance->usart_handle, message, tx_len) !=
        HAL_OK) {
      ret = RETURN_ERROR;
    }
    break;
  ...
  case USART_TRANSFER_BLOCKING:
    if (HAL_UART_Transmit(instance->usart_handle, message, tx_len, 0xFF) !=
        HAL_OK) {
      ret = RETURN_ERROR;
    }
    break;
                                ...
                                uint16_t size) // todo  双缓存区如何接收
{
  for (uint8_t i = 0; i < id_cnt;
    ...
    if (huart == usart_device[i]->usart_handle) { // call the callback function
                                                  // if it is not NULL
