...
void canFilterConfig(Can_Device_t* device) {
    ...
    can_filter_config.FilterConfig =
...
Can_Device_t* canDeviceRegister(Can_Register_t* reg) {
    ...
    Can_Device_t* instance =
        (Can_Device_t*)malloc(sizeof(Can_Device_t));  // 分配空间
    memset(instance, 0,
    ...
    instance->tx_config.ErrorStateIndicator =
        FDCAN_ESI_ACTIVE;  // 设置错误状态指示
    instance->tx_config.BitRateSwitch = FDCAN_BRS_OFF;  // 不开启可变波特率
    instance->tx_config.TxEventFifoControl =
...
Return_t canSendMessage(Can_Device_t* instance, uint8_t* message) {
    ...
    if (HAL_CAN_AddTxMessage(instance->can_handle, &instance->tx_config,
                             message, &instance->tx_mailbox) == HAL_ERROR)
        return RETURN_ERROR;
    return RETURN_SUCCESS;
#elif defined(FDCAN_DEVICE)
    memcpy(instance->tx_buff, message, instance->tx_config.DataLength >> 16);
    if (HAL_FDCAN_AddMessageToTxFifoQ(
            instance->can_handle, &instance->tx_config, message) == HAL_ERROR)
        return RETURN_ERROR;
    return RETURN_SUCCESS;
#endif
}
