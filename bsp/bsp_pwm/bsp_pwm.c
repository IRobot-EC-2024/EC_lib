 ...
 * @LastEditTime : 2024-04-08 13:45:04
 * @Description  :
 * @Filename     : bsp_pwm.c
...
#define PCLK1_MULT                                        \
    (D1CorePrescTable[(RCC->CFGR & RCC_D2CFGR_D2PPRE1) >> \
                      RCC_D2CFGR_D2PPRE1_Pos] == 0        \
         ? 1                                              \
         : 2)
#define PCLK2_MULT                                        \
    (D1CorePrescTable[(RCC->CFGR & RCC_D2CFGR_D2PPRE2) >> \
...
static uint32_t PWMSelectTclk(
    TIM_HandleTypeDef* htim);  // 通过与APB1和APB2比较来猜测pwm通道的时钟频率
...
PWM_Device_t* pwmRegister(PWM_Register_t* config) {
    if (id_cnt >=
        PWM_DEVICE_CNT)  // 超过最大实例数,考虑增加或查看是否有内存泄漏
        while (1)
            ;
...
void pwmSetPeriod(PWM_Device_t* pwm, fp32 period) {
    __HAL_TIM_SetAutoreload(
        pwm->htim,
        period * ((pwm->tclk) / (pwm->htim->Init.Prescaler + 1)) / 1000);
}
